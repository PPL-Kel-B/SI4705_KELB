...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm p-8">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-gray-900">Tambah Menu Makanan</h1>
            <p class="text-gray-500 mt-2">Isi semua informasi menu makanan baru Anda di bawah</p>
        </div>
        ...
        <form action="{{ route('foods.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="kategori"
                        ...
                        <option value="NASI KOTAK" {{ old('kategori') == 'NASI KOTAK' ? 'selected' : '' }}>Nasi Kotak</option>
                        <option value="PASTA" {{ old('kategori') == 'PASTA' ? 'selected' : '' }}>Pasta</option>
                        <option value="SALAD" {{ old('kategori') == 'SALAD' ? 'selected' : '' }}>Salad</option>
                        <option value="WESTERN" {{ old('kategori') == 'WESTERN' ? 'selected' : '' }}>Western</option>
                        <option value="ASIAN" {{ old('kategori') == 'ASIAN' ? 'selected' : '' }}>Asian</option>
                        <option value="SOUP" {{ old('kategori') == 'SOUP' ? 'selected' : '' }}>Soup</option>
            ...
            <div>
                ...
                <textarea id="deskripsi"
                          ...
                          class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-green-500 transition resize-none @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div id="file-preview" class="mt-4 hidden">
                    ...
                    <button type="button" onclick="clearFile()" class="mt-2 text-sm text-red-500 hover:text-red-700">Hapus Foto</button>
                </div>
                @error('foto')
                    <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                @enderror
