...
<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
    ...
    {{-- Search --}}
    <form method="GET" action="{{ route('kelolamasterdata.index') }}"
        ...
        <div class="relative flex-1 sm:w-80">
            ...
            <input type="text"
                   ...
                   class="w-full bg-white border border-gray-200 rounded-2xl pl-11 pr-4 py-3.5 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand/40 focus:border-brand shadow-sm transition">
        </div>
        <button type="submit"
                class="ml-2 bg-white border border-gray-200 rounded-2xl px-4 py-3.5 text-sm font-medium text-gray-600 hover:bg-brand-light hover:text-brand hover:border-brand shadow-sm transition">
            Cari
        </button>
        @if(request('search'))
        <a href="{{ route('kelolamasterdata.index') }}"
           class="ml-2 bg-white border border-gray-200 rounded-2xl px-4 py-3.5 text-sm font-medium text-gray-500 hover:text-red-500 shadow-sm transition">
            Reset
        </a>
        @endif
    </form>
    ...
    {{-- Tambah Button --}}
    <a href="{{ route('kelolamasterdata.create') }}"
       id="btn-tambah-master"
       class="flex items-center space-x-2 bg-brand hover:bg-brand-dark text-white font-semibold px-6 py-3.5 rounded-2xl shadow-lg hover:shadow-brand/40 transition-all duration-200 whitespace-nowrap"
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
    ...
    <div class="food-card bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl cursor-pointer">

        {{-- Food Image --}}
        <div class="food-img-wrap relative bg-gradient-to-br from-gray-100 to-gray-200 w-full"
            ...
            <div class="absolute top-3 right-3">
                @if($makanan->Status === 'Tersedia')
                    <span class="bg-green-500 text-white text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wide shadow">
                        Tersedia
                    </span>
                @elseif($makanan->Status === 'Habis')
                    <span class="bg-red-500 text-white text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wide shadow">
                        Habis
                    </span>
                @else
                    <span class="bg-gray-500 text-white text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wide shadow">
                        {{ $makanan->Status ?? 'N/A' }}
                    </span>
                @endif
            </div>
        ...
        <div class="p-5">
            ...
            <div class="flex items-center space-x-2">
                {{-- Edit --}}
                <a href="{{ route('kelolamasterdata.edit', $makanan->MakananID) }}"
                ...
                {{-- Delete --}}
                <form action="{{ route('kelolamasterdata.destroy', $makanan->MakananID) }}"
                      method="POST"
                      class="delete-form"
