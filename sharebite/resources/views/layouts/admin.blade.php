...
<head>
    ...
    <title>@yield('title', 'Admin Dashboard') - ShareBite</title>
...
<body class="text-gray-800 antialiased" x-data="{ sidebarOpen: false, sidebarMinimized: false, showLogoutModal: false }">
    ...
    <aside :class="{
        ...
        <nav class="flex-1 overflow-y-auto px-4 space-y-2">
            ...
            <a href="{{ route('admin.dashboard') }}"
                class="flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all {{ request()->routeIs('admin.dashboard') ? 'bg-[#eefcf4] text-[#0a2e1f] font-bold' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700 font-semibold' }}">
                <svg xmlns="http://www.w3.org/2000/svg"
                    class="h-5 w-5 shrink-0 {{ request()->routeIs('admin.dashboard') ? 'text-[#1cb764]' : '' }}"
            ...
            <!-- Manajemen Pengguna -->
            <a href="{{ route('admin.manajemen_pengguna') }}"
                class="flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all {{ request()->routeIs('admin.manajemen_pengguna') ? 'bg-[#eefcf4] text-[#0a2e1f] font-bold' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700 font-semibold' }}">
                <svg xmlns="http://www.w3.org/2000/svg"
                    class="h-5 w-5 shrink-0 {{ request()->routeIs('admin.manajemen_pengguna') ? 'text-[#1cb764]' : '' }}"
                    fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
                </svg>
                <span x-show="!sidebarMinimized" class="whitespace-nowrap">Manajemen User</span>
            </a>

            <!-- Chat -->
            <a href="{{ route('admin.chat') }}"
                class="flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all {{ request()->routeIs('admin.chat') ? 'bg-[#eefcf4] text-[#0a2e1f] font-bold' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700 font-semibold' }}">
                <svg xmlns="http://www.w3.org/2000/svg"
                    class="h-5 w-5 shrink-0 {{ request()->routeIs('admin.chat') ? 'text-[#1cb764]' : '' }}" fill="none"
                    viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
                </svg>
                <span x-show="!sidebarMinimized" class="whitespace-nowrap">Chat</span>
            </a>
        </nav>
        ...
        <div class="p-4 border-t border-gray-50 mb-4 mt-2">
            <button @click="showLogoutModal = true"
                class="flex items-center gap-4 px-4 py-3.5 w-full rounded-xl text-red-600 hover:bg-red-50 transition-colors font-bold text-sm text-left">
    ...
    <div :class="{
        ...
        <!-- Top Navbar -->
        <header class="h-16 flex items-center justify-between px-6 lg:px-10">
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="relative animate-fade-in" x-data="{ open: false }">
                    <button @click="open = !open" @click.outside="open = false"
                        class="relative w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 hover:text-gray-700 hover:bg-gray-200 transition focus:outline-none">
                                ...
                                @forelse(auth()->user()->notifications()->take(5)->get() as $notification)
                                    <div class="p-4 border-b border-gray-50 hover:bg-gray-50 transition flex gap-3 relative {{ is_null($notification->read_at) ? 'bg-[#eefcf4]/30' : '' }}">
                                        ...
                                        <div class="shrink-0">
                                            ...
                                            @elseif(($notification->data['type'] ?? '') === 'register')
                ...
                <button
                    class="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 hover:text-gray-700 hover:bg-gray-200 transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <!-- Main Content Area -->
        <main class="flex-1 px-6 lg:px-10 pb-10">
            @yield('content')
        </main>
    </div>
