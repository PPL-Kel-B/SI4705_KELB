...
<body class="text-gray-800 antialiased" x-data="{ sidebarOpen: false, sidebarMinimized: false, showLogoutModal: false }">
    ...
    <aside
        ...
        <nav class="flex-1 overflow-y-auto px-4 space-y-2">
            ...
            <a href="{{ route('user.dashboard') }}"
                class="flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all {{ request()->routeIs('user.dashboard') ? 'bg-[#eefcf4] text-[#1cb764] font-semibold' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700 font-medium' }}">
            ...
            <a href="{{ route('user.riwayat') }}"
                class="flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all {{ request()->routeIs('user.riwayat') ? 'bg-[#eefcf4] text-[#1cb764] font-semibold' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700 font-medium' }}">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="{{ route('user.chat') }}"
                class="flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all {{ request()->routeIs('user.chat') ? 'bg-[#eefcf4] text-[#0a2e1f] font-bold' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700 font-semibold' }}">
                <svg xmlns="http://www.w3.org/2000/svg"
                    class="h-5 w-5 shrink-0 {{ request()->routeIs('user.chat') ? 'text-[#1cb764]' : '' }}" fill="none"
                    viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
                </svg>
                <span x-show="!sidebarMinimized" class="whitespace-nowrap">Chat Admin</span>
            </a>

            <!-- Profile -->
            <a href="{{ route('user.profile') }}"
                class="flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all {{ request()->routeIs('user.profile') ? 'bg-[#eefcf4] text-[#1cb764] font-semibold' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700 font-medium' }}">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
                <span x-show="!sidebarMinimized" class="whitespace-nowrap">Profile</span>
            </a>

            <!-- Pengaturan -->
            <a href="{{ route('user.pengaturan') }}"
                class="flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all {{ request()->routeIs('user.pengaturan') ? 'bg-[#eefcf4] text-[#1cb764] font-semibold' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700 font-medium' }}">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div :class="{
        ...
        <!-- Header -->
        <header class="h-20 flex items-center justify-between px-6 lg:px-10">
                ...
                <div class="relative animate-fade-in" x-data="{ open: false }">
                    <button @click="open = !open" @click.outside="open = false"
                        ...
                        @if(auth()->check() && auth()->user()->unreadNotifications->count() > 0)
                            <span class="absolute top-0 right-0 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-[#F4F7F5]"></span>
                        @endif
                    </button>
                            ...
                            @if(auth()->check())
                                @forelse(auth()->user()->notifications()->take(5)->get() as $notification)
                                    <div class="p-4 border-b border-gray-50 hover:bg-gray-50 transition flex gap-3 relative {{ is_null($notification->read_at) ? 'bg-[#eefcf4]/30' : '' }}">
                                        ...
                                        <div class="shrink-0">
                                            @if(($notification->data['type'] ?? '') === 'menu')
                ...
                <div class="hidden sm:flex items-center gap-3">
                    <div class="text-right">
                        <p class="text-[11px] text-gray-500 font-medium uppercase tracking-wider">
                            {{ Auth::check() ? Auth::user()->role : 'Komunitas' }}</p>
        ...
        <!-- Main Content Area -->
        <main class="flex-1 px-6 lg:px-10 pb-6 mt-2">
            @yield('content')
        </main>
    </div>
