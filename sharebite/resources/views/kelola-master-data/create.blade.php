...
@section('title', 'Tambah Master Data Makanan')

@section('page-title', 'Tambah Master Data Makanan')
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-3xl shadow-sm overflow-hidden" style="border: 1px solid #e8f0ed;">
        ...
        <div class="bg-gradient-to-r from-brand to-brand-dark px-8 py-6">
            <div class="flex items-center space-x-4">
                <div class="w-16 h-16 rounded-2xl bg-white/20 flex items-center justify-center flex-shrink-0" id="preview-header-wrap">
                    <svg class="w-8 h-8 text-white" id="preview-header-placeholder" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                              d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                    </svg>
                    <img id="preview-header-img" src="" alt="Preview" class="w-full h-full object-cover rounded-2xl hidden">
                </div>
                <div>
                    <p class="text-white/70 text-xs font-semibold uppercase tracking-widest mb-0.5">Master Data Baru</p>
                    <h2 class="text-white text-xl font-extrabold">Tambah Makanan</h2>
                    <p class="text-white/60 text-xs mt-0.5">Isi semua field yang diperlukan</p>
                </div>
            </div>
        </div>

        {{-- Form --}}
        <form action="{{ route('kelola-master-data.store') }}"
              method="POST"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {{-- Jumlah Porsi --}}
                <div>
                    <label for="Jumlah_porsi" class="block text-sm font-semibold text-gray-700 mb-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="flex space-x-3">
                        ...
                        <label class="flex-1 cursor-pointer">
                            <input type="radio"
                                   ...
                                   value="{{ $status }}"
                                   class="sr-only peer"
                                   {{ old('Status', 'Tersedia') === $status ? 'checked' : '' }}>
                ...
                {{-- Batas Waktu --}}
                <div>
                    <label for="Batas_waktu_pengambilan" class="block text-sm font-semibold text-gray-700 mb-2">
                        Batas Waktu Pengambilan <span class="text-red-500">*</span>
                    </label>
                    <input type="date"
                           id="Batas_waktu_pengambilan"
                           name="Batas_waktu_pengambilan"
                           value="{{ old('Batas_waktu_pengambilan') }}"
                           min="{{ date('Y-m-d') }}"
            ...
            <div>
                ...
                <div id="drop-zone"
                    ...
                    <div id="image-preview" class="hidden flex-col items-center">
                        ...
                        <p class="text-xs text-brand mt-1">Gambar dipilih — klik untuk ganti</p>
                    </div>
                </div>
                @error('Foto')
                <p class="text-xs text-red-500 mt-1.5">{{ $message }}</p>
            ...
            <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                ...
                <button type="submit"
                        id="save-btn"
                        class="flex items-center space-x-2 bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 text-white font-semibold px-8 py-3 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-200">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="mt-4 flex items-start space-x-2 bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4">
        ...
        <p class="text-xs text-amber-700">
            ...
            Data yang disimpan akan langsung tampil di daftar Master Data Makanan.
        </p>
    </div>
...
<script>
    function previewImage(event) {
        ...
        const headerImg    = document.getElementById('preview-header-img');
        const headerPlaceholder = document.getElementById('preview-header-placeholder');
