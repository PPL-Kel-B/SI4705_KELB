...
@section('content')
    <div class="max-w-7xl mx-auto -mt-4" x-data="{ showApproveModal: false, showRejectModal: false, notes: '' }">
        ...
        <div class="flex items-center text-xs font-bold text-gray-400 uppercase tracking-widest mb-6 gap-2">
            <a href="{{ route('admin.manajemen_pengguna') }}" class="hover:text-[#1cb764] transition-colors">Manajemen
                Pengguna</a>
            <span>/</span>
            <a href="{{ route('admin.manajemen_pengguna', ['tab' => 'verifikasi_nib']) }}"
                class="hover:text-[#1cb764] transition-colors">Verifikasi NIB</a>
            <span>/</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-7 bg-white rounded-3xl p-8 shadow-sm border border-gray-50 flex flex-col">
                ...
                <div
                    class="flex-1 bg-gray-50 rounded-2xl overflow-hidden border border-gray-100 min-h-[500px] flex flex-col">
                    @if($profile->nib_file)
                        ...
                        @if(in_array($ext, ['pdf']))
                            ...
                            <div class="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ asset('storage/' . $profile->nib_file) }}" target="_blank" 
                                   class="bg-[#1cb764] text-white px-4 py-2 rounded-xl font-bold text-xs shadow-lg flex items-center gap-2 hover:bg-[#0a2e1f]">
                        ...
                        @else
                            <img src="{{ asset('storage/' . $profile->nib_file) }}" alt="Dokumen NIB"
            ...
            <div class="lg:col-span-5 flex flex-col gap-6">
                ...
                <div class="bg-white rounded-3xl p-8 shadow-sm border border-gray-50 flex-1">
                    ...
                    <div class="space-y-6">
                        {{-- Nama Usaha --}}
                        <div>
                            <p class="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest mb-1">Nama Usaha
                            </p>
                        ...
                        {{-- Jenis Usaha --}}
                        <div>
                            <p class="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest mb-2">Jenis Usaha
                            </p>
                            <span
                                class="inline-flex items-center px-3 py-1 bg-[#eefcf4] text-[#1cb764] rounded-full text-xs font-bold uppercase tracking-wider">
                                {{ $profile->jenis_usaha ?? '-' }}
                            </span>
                        </div>
                        ...
                        {{-- Alamat --}}
                        <div>
                            <p class="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest mb-2">Alamat
                                Lengkap</p>
                            <div class="flex gap-2">
                                <svg class="w-4 h-4 text-[#1cb764] shrink-0 mt-0.5" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <p class="text-sm text-gray-600 leading-relaxed">
                                    {{ $profile->user->alamat ?? 'Alamat belum diperbarui' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="pt-6 border-t border-gray-50">
                            <p class="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest mb-3">Reviewer
                                Notes</p>
                            <textarea x-model="notes" rows="4" placeholder="Berikan catatan verifikasi jika diperlukan..."
        ...
        <div x-show="showRejectModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4" x-cloak>
            ...
            <div class="relative bg-white rounded-3xl p-10 max-w-sm w-full shadow-2xl text-center"
                ...
                <p class="text-sm text-gray-500 mb-8 leading-relaxed">
                    Aksi ini akan menolak pengajuan NIB. Pastikan Anda sudah mengisi Reviewer Notes sebagai alasan
                    penolakan.
                </p>
                <form action="{{ route('admin.manajemen_pengguna.process_nib', $profile->user_id) }}" method="POST"
                    class="flex gap-3">
                    @csrf
        ...
        <div x-show="showApproveModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4" x-cloak>
            ...
            <div class="relative bg-white rounded-3xl p-10 max-w-sm w-full shadow-2xl text-center"
                ...
                x-transition:enter-end="opacity-100 scale-100">
                <div
                    class="w-16 h-16 bg-[#eefcf4] rounded-full flex items-center justify-center text-[#1cb764] mx-auto mb-6">
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7" />
                    </svg>
                </div>
